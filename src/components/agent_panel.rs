...
use crate::models::{AgentEvent, AgentProvider, ModelCacheEntry, SshConnection, StreamDelta};
...
pub fn AgentPanel(
    ...
    let send = move || {
        ...
        spawn_local(async move {
            if let Err(err) = agent_send(&session, &provider_id, model, &message, &on_event).await
            {
                running.set(false);
